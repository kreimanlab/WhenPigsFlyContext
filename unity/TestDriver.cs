...
namespace StoryGenerator
{
    ...
    public class TestDriver : MonoBehaviour
    {
        ...
        //mengmi initialize: record past prefabs that have been modified
        public List<int> PrefabIDList;
        public List<Vector3> PrefabPosList;
        public List<Vector3> PrefabScaList;
        public List<Vector3> PrefabRotList;

        // for texture randomization
        private Texture[] random_textures;
        ...
        void Start()
        {
            ...
            //Mengmi: change skybox to all black
            Debug.Log(Application.dataPath);
            Material skyblack = Resources.Load("Material/black", typeof(Material)) as Material;
            RenderSettings.skybox = skyblack;

            // initialization for texture randomization
            random_textures = Resources.LoadAll<Texture>("Material/ObjMaterials/");
            Debug.Log("There are numbers of materials total: ");
            Debug.Log(random_textures.Length.ToString());
        ...
        //Mengmi: change bathroom texture in apartment 0 and 6; hard coded
        void ChangeBathroomTexture(int apartmentid)
        {
            var texture_floor = Resources.Load<Texture2D>(string.Concat("Material/floor_", apartmentid.ToString()));
            var texture_wall = Resources.Load<Texture2D>(string.Concat("Material/wall_", apartmentid.ToString()));
            var texture_counter = Resources.Load<Texture2D>(string.Concat("Material/bathroomCounter_", apartmentid.ToString()));

            foreach (Transform room in transform)
            {
                if (room.gameObject.name.Contains("Bathroom"))
                {
                    foreach (Transform objclass in room)
                    {
                        if (objclass.gameObject.name == "Walls" || objclass.gameObject.name == "Ceiling")
                        {
                            foreach (Transform obj in objclass)
                            {
                                ...
                                if (renderer != null)
                                {
                                    ...
                                    {
                                        material.mainTexture = texture_wall;
                                    }

                                }
                                else
                                {
                                    Renderer[] childrendererlist = obj.gameObject.GetComponentsInChildren<Renderer>();
                                    if (childrendererlist != null)
                                    {
                                        foreach (Renderer childrenderer in childrendererlist)
                                        {
                        ...
                        else if (objclass.gameObject.name == "Floor")
                        {
                            foreach (Transform obj in objclass)
                            {
                                Renderer renderer = obj.gameObject.GetComponent<Renderer>();
                                if (renderer != null)
                                {
                                    foreach (Material material in renderer.materials)
                                    {
                                        material.mainTexture = texture_floor;
                                    }
                        ...
                        {
                            if (objclass.gameObject.name == "Furniture")
                            {
                                foreach (Transform obj in objclass)
                                {
                                    if (obj.gameObject.name.Contains("counter"))
                                    {
                                        Renderer renderer = obj.gameObject.GetComponent<Renderer>();
                                        if (renderer != null)
                                        {
                                            foreach (Material material in renderer.materials)
                                            {
                                                material.mainTexture = texture_counter;
                                            }
        ...
        IEnumerator ProcessNetworkRequest()
        {
            ...
            while (true) {
                ...
                else if (networkRequest.action == "add_camera")
                {
                    ...
                    // Mengmi: modify camera properties :
                    new_camera.nearClipPlane = 0.001f;
                    //new_camera.farClipPlane = 100f;
                ...
                else if (networkRequest.action == "MMdeleteCamera")
                {
                    //not implemented yet; has problems
                    //remove the actual camera gameobject
                    //Camera newcam = cameras[cameras.Count - 1];
                    //Debug.Log("found new camera");
                    //Debug.Log(newcam);
                    //Destroy(newcam);
                    ...
                    response.message = "New camera has been deleted";
                    response.success = true;
                }
                else if (networkRequest.action == "modify_MMobjectTransform")
                {

                    ModifyMMobjConfig MMobj_config = JsonConvert.DeserializeObject<ModifyMMobjConfig>(networkRequest.stringParams[0]);
                    Debug.Log(MMobj_config.nodeid.ToString());
                    Debug.Log(MMobj_config.position.ToString("F4"));
                    Debug.Log(MMobj_config.scaling.ToString("F4"));
                    Debug.Log(MMobj_config.rotation.ToString("F4"));

                    //Mengmi here: e.g. 54 "towel"
                    int targetid = MMobj_config.nodeid;
                    ...
                    foreach (EnvironmentObject tf_obj in currentGraph.nodes)
                    {
                        if (tf_obj.transform != null)
                        {
                            if (tf_obj.id == targetid)
                            {
                                Debug.Log("I found you!");
                                //destroy this object
                                //Destroy(tf_obj.gameObject);
                                Debug.Log(tf_obj.id.ToString());
                                //record object's pose to be modified
                                float roll_x = MMobj_config.rotation.x; float yaw_y = MMobj_config.rotation.y; float pitch_z = MMobj_config.rotation.z;
                                //float roll_x = 10f; float yaw_y = 0f; float pitch_z = 0f;

                                //Debug.Log("starting to add to list1");
                                //Debug.Log(PrefabNameList.Count);

                                if (!PrefabIDList.Contains(targetid))
                                {
                                    PrefabIDList.Add(targetid);
                                    PrefabPosList.Add(tf_obj.transform.gameObject.transform.position);
                                    PrefabScaList.Add(tf_obj.transform.gameObject.transform.localScale);
                                    PrefabRotList.Add(new Vector3(0f, 0f, 0f)); //dont change; wrt itself; 
                                }

                                //modify object's pose as desired by certian offsets
                                int indexprefab = PrefabIDList.IndexOf(targetid);

                                //Debug.Log("print current stored prefab infor (pos, sca, rot):");
                                //Debug.Log(PrefabPosList[indexprefab].ToString("F4"));
                                //Debug.Log(PrefabScaList[indexprefab].ToString("F4"));
                                //Debug.Log(PrefabRotList[indexprefab].ToString("F4"));

                                //rotate in sequence along diff axis
                                tf_obj.transform.gameObject.transform.Rotate(PrefabRotList[indexprefab].x + roll_x, 0f, 0f);
                                tf_obj.transform.gameObject.transform.Rotate(0f, PrefabRotList[indexprefab].y + yaw_y, 0f, Space.Self);
                                tf_obj.transform.gameObject.transform.Rotate(0f, 0f, PrefabRotList[indexprefab].z + pitch_z, Space.Self);

                                tf_obj.transform.gameObject.transform.position = PrefabPosList[indexprefab] + MMobj_config.position;
                                tf_obj.transform.gameObject.transform.localScale = PrefabScaList[indexprefab] + MMobj_config.scaling;

                                PrefabRotList[indexprefab] = new Vector3(-roll_x, -yaw_y, -pitch_z);

                                //let the most recent camera look at the newly added object
                                cameras[cameras.Count - 1].transform.LookAt(tf_obj.transform);

                                //reset graph nodes based on updated transform
                                currentGraph = null;
                            }

                        }
                    }
                    response.message = "MMobj transform has been modified";
                    response.success = true;
                }
                else if (networkRequest.action == "lookat_MMobjectTransform")
                {
                    ...
                    response.message = "MMobj has been looked at";
                    response.success = true;
                }
                else if (networkRequest.action == "changeMaterial_MMobjectTransform")
                {

                    //int materialid = UnityEngine.Random.Range(0, random_textures.Length);

                    ModifyMMobjConfigMaterial MMobj_config = JsonConvert.DeserializeObject<ModifyMMobjConfigMaterial>(networkRequest.stringParams[0]);
                    Debug.Log(MMobj_config.nodeid.ToString());
                    Debug.Log(MMobj_config.position.ToString("F4"));

                    int targetid = MMobj_config.nodeid;
                    int materialid = MMobj_config.materialid;
                    if (currentGraph == null)
                    {
                        EnvironmentGraphCreator graphCreator = new EnvironmentGraphCreator(dataProviders);
                        currentGraph = graphCreator.CreateGraph(transform);
                    ...
                    foreach (EnvironmentObject tf_obj in currentGraph.nodes)
                    {
                        if (tf_obj.transform != null)
                        {
                            if (tf_obj.id == targetid)
                            {
                                Debug.Log("About to change material of this object!");
                                //let the most recent camera look at the newly added object
                                cameras[cameras.Count - 1].transform.LookAt(tf_obj.transform);

                                //let the most recent camera look at the newly added object
                                Renderer renderer = tf_obj.transform.gameObject.GetComponent<Renderer>();
                                if (renderer != null)
                                {
                                    foreach (Material material in renderer.materials)
                                    {
                                        material.mainTexture = random_textures[materialid];
                                    }

                                }
                                else
                                {
                                    Renderer[] childrendererlist = tf_obj.transform.gameObject.GetComponentsInChildren<Renderer>();
                                    if (childrendererlist != null)
                                    {
                                        foreach (Renderer childrenderer in childrendererlist)
                                        {
                    ...
                    response.message = "Material of MMobj has been changed";
                    response.success = true;
                }
                else if (networkRequest.action == "reset_MMobjectTransform")
                {
                    //Mengmi here: e.g. "MOD_PRO_Coffeemaker_01_Pot_01"
                    //Debug.Log("Revert back to its original gameobject pose");
                    //Debug.Log(PrefabNameList.Count);
                    ...
                    int count = 0;
                    foreach (int prefabid in PrefabIDList)
                    {
                        foreach (EnvironmentObject tf_obj in currentGraph.nodes)
                        {
                            if (tf_obj.transform != null)
                            {
                                if (tf_obj.id == prefabid)
                                {

                                    //Debug.Log("I found you in reset_MMobjectTransform!");

                                    //Debug.Log("print current stored prefab infor (pos, sca, rot) before reset:");
                                    //Debug.Log(PrefabPosList[count].ToString("F4"));
                                    //Debug.Log(PrefabScaList[count].ToString("F4"));
                                    //Debug.Log(PrefabRotList[count].ToString("F4"));

                                    tf_obj.transform.gameObject.transform.position = PrefabPosList[count];
                                    tf_obj.transform.gameObject.transform.localScale = PrefabScaList[count];

                                    //rotate back in opposite sequence z, y, x axis last
                                    tf_obj.transform.gameObject.transform.Rotate(0f, 0f, PrefabRotList[count].z, Space.Self);
                                    tf_obj.transform.gameObject.transform.Rotate(0f, PrefabRotList[count].y, 0f, Space.Self);
                                    tf_obj.transform.gameObject.transform.Rotate(PrefabRotList[count].x, 0f, 0f, Space.Self);
                    ...
                    //clear all history prefabs modified
                    PrefabIDList.Clear();
                    PrefabPosList.Clear();
                    PrefabScaList.Clear();
                    PrefabRotList.Clear();

                    //delete the newly created camera object
                    //MM_DeleteCamera();

                    //reset current graph
                    currentGraph = null;
                    response.message = "reset all past modified prefab transforms";
                    response.success = true;
                ...
                else if (networkRequest.action == "instance_colors_and_prefab")
                {
                    if (currentGraph == null)
                    {
                        EnvironmentGraphCreator graphCreator = new EnvironmentGraphCreator(dataProviders);
                    ...
                    response.success = true;
                    response.message = JsonConvert.SerializeObject(GetInstanceColoringAndPrefab(currentGraph.nodes));
                }
                ...
                else if (networkRequest.action == "reset")
                {
                    ...
                    if (networkRequest.intParams?.Count > 0)
                    {
                        ...
                        if (sceneIndex >= 0 && sceneIndex < SceneManager.sceneCountInBuildSettings)
                        {
                            ...
                            {
                                ChangeBathroomTexture(sceneIndex);
                            }*/
                            yield break;
                        }
                        else
                ...
                else if (networkRequest.action == "setBathroomTexture")
                {
                    int sceneIndex = networkRequest.intParams[0];
                    if (sceneIndex == 0 || sceneIndex == 6 || sceneIndex == 5)
                    {
                        ChangeBathroomTexture(sceneIndex);
                    }
        ...
        private Dictionary<string, float[]> GetInstanceColoring(IList<EnvironmentObject> graphNodes)
        {
            ...
            foreach (EnvironmentObject eo in graphNodes) {
                if (eo.transform == null) {
                    ...
                    result[eo.prefab_name] = new float[] { 1.0f, 1.0f, 1.0f };
                } else {
                    ...
                    if (ColorEncoding.instanceColor.TryGetValue(instId, out instColor)) {
                        ...
                        result[eo.prefab_name] = new float[] { instColor.r, instColor.g, instColor.b };
                    } else {
                        //result[eo.id] = new float[] { 1.0f, 1.0f, 1.0f };
                        result[eo.prefab_name] = new float[] { 1.0f, 1.0f, 1.0f };
                    }
        ...
        //get instance coloring based on ID and prefab; prefab name is not unique; multiple IDs could have the same prefabname
        private Dictionary<int, (float[], string)> GetInstanceColoringAndPrefab(IList<EnvironmentObject> graphNodes)
        {
            Dictionary<int, (float[], string)> result = new Dictionary<int, (float[], string)>();

            foreach (EnvironmentObject eo in graphNodes)
            {
                if (eo.transform == null)
                {
                    result[eo.id] = (new float[] { 1.0f, 1.0f, 1.0f }, eo.prefab_name);
                }
                else
                {
                    int instId = eo.transform.gameObject.GetInstanceID();
                    ...
                    if (ColorEncoding.instanceColor.TryGetValue(instId, out instColor))
                    {
                        result[eo.id] = (new float[] { instColor.r, instColor.g, instColor.b }, eo.prefab_name);
                    }
                    else
                    {
                        result[eo.id] = (new float[] { 1.0f, 1.0f, 1.0f }, eo.prefab_name);
    ...
    public class ModifyMMobjConfig
    {
        public int nodeid;
        public Vector3 scaling = new Vector3(0.0f, 0.0f, 0.0f);
        public Vector3 rotation = new Vector3(0.0f, 0.0f, 0.0f);
        public Vector3 position = new Vector3(0.0f, 0.0f, 0.0f);
    }

    public class ModifyMMobjConfigMaterial
    {
        public int nodeid;
