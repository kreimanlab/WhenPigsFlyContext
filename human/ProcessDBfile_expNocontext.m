...
csvname = 'csv/results_NoContext.csv';
system(['sqlite3 -header -csv db/expNoContext.db "select * from expNoContext;" > ' csvname]);

fid = fopen(csvname);
out = textscan(fid,'%s');
strtotal = out{1,1};
pattern = 'http://kreimanlab.com/mengmiMturkHost/VirtualHome/keyframe_VH_NoContext_gif/';

savefilename = 'Mat/mturk_expNoContext.mat';
...
for i = 1:length(strtotal)
    ...
    ans.imageID = imageID;
    if strcmp(imageID(end-5:end), 'sp.gif')
        ans.contextcond = 2; %blakc and white
    else
        ans.contextcond = 1; %pure grey
    end
