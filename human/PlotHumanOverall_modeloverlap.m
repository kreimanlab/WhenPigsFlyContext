...
modelclass = {'wineglass','mug','cutleryknife','dishbowl','apple',...
    'poundcake','pie','mouse','remotecontrol','keyboard','cellphone',...
'microwave','book','toothbrush','pillow','towel'};
...
for exp = 1:length(expnamelist)
    ...
    if strcmp(expname,'expAnomaly')
        ...
        classnamelist = extractfield(VHhumanStats, 'classname');
        overall = extractfield(VHhumanStats(find(condwalllist == 0 & dvalist <= 2 & ismember(classnamelist, modelclass) )), 'GIFcorrect');   
        ...
        condwalllist = extractfield(VHhumanStats, 'condWall');
        classnamelist = extractfield(VHhumanStats, 'classname');
        overall = extractfield(VHhumanStats(find(condwalllist == 1 & dvalist <= 2 & ismember(classnamelist, modelclass))), 'GIFcorrect');   
        ...
        overall = extractfield(VHhumanStats(find(dvalist <= 2 & ismember(classnamelist, modelclass))), 'GIFcorrect');   
        mturk_mean = [mturk_mean nanmean(overall)];
        mturk_std = [mturk_std nanstd(overall)/sqrt(length(overall))];
...
save(['Mat/humanresult.mat'],'mturk_mean','mturk_std');
...
print(hb,['Figures/fig_human_mturk_overall_overlapmodel' printpostfix],printmode,printoption);
