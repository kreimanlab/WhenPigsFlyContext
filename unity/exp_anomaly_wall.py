...
ThresNumInst = 8 #at least 15 instances on the picture
...
stimulusdirname = rootdir + 'stimulus_anomaly_wall'
jasondirname = rootdir + 'jason_anomaly_wall'
...
for fileid, filename in enumerate(os.listdir(graphdirname)):
    ...
    try:
        ...
        #PICKLE = pickle.load( open( graphdirname + '/remotecontrol_apartment_2_room_kitchen_Fur_wall.pkl',"rb" ) )    
        
        if PICKLE['dest_surf'] != 'wall':
            continue
        ...
        destroomnode = PICKLE['destnode_wall']
        destwallnode = PICKLE['destnode_room']
        movenode_offset = computeMoveNodeOffset_anomaly(destwallnode, destroomnode, node)
    ...
    ##############################################################
        ...
        try:
            for circ_coor in circ: #camera revolving along a circle cnetered at target
                ...
                cam_pos = destwallnode['bounding_box']['center'].copy()   #yaxis is always 0 meter high  
                cam_pos[0] = cam_pos[0] + xmov
                cam_pos[1] = cam_pos[1]/2 + camYStepSz #move up 1 meter
    ...
    #                continue
                ...
                pos = movenode_offset #move object 1 meter up
