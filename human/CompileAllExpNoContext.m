...
expname = 'expNoContext';
load(['Mat/mturk_expNoContext.mat']);
...
for i = startingMturkData:length(mturkData)
    ...
    infor = importdata(['/home/mengmi/Projects/Proj_context3/VirtualHome/matlab/MturkSets/exp_nocontext/mturkSet_' num2str(type) '.txt']);
...
for i = 1:length(mturkData)
    ...
    Hcontextcond = [Hcontextcond extractfield(mturkData(i).answer,'contextcond')];
end
...
for i = 1:length(VHhumanStats)
    ...
    VHhumanStats(i).ContextCond = Hcontextcond(indselected);
end
...
%save(['Mat/mturk_' expname '_results.mat'],'VHhumanStats','overall');

%display accuracy of humans based on class and condition (grey bg)
overall_condG = [];
for i = 1:length(classuniqueidlist)
    labelid = classuniqueidlist(i);
    selectedind = find(classidlist == labelid);
    ...
    label = VHhumanStats(selectedind(1)).classname;
    meanaccu = nanmean(HCorrectlist(find(Hlabelidlist == labelid & Hcontextcond == 1)));
    overall_condG = [overall_condG meanaccu];
    display(['Accuracy (' label ') = ' num2str(meanaccu)]);
end
display(['overall = ' num2str(nanmean(overall_condG)) ';rmse = ' num2str(nanstd(overall_condG)/sqrt(length(overall_condG)))]);

%display accuracy of humans based on class and condition (salt and pepper bg)
overall_condSP = [];
for i = 1:length(classuniqueidlist)
    labelid = classuniqueidlist(i);
    selectedind = find(classidlist == labelid);
    ...
    label = VHhumanStats(selectedind(1)).classname;
    meanaccu = nanmean(HCorrectlist(find(Hlabelidlist == labelid  & Hcontextcond == 2)));
    overall_condSP = [overall_condSP meanaccu];
    display(['Accuracy (' label ') = ' num2str(meanaccu)]);
end
display(['overall = ' num2str(nanmean(overall_condSP)) ';rmse = ' num2str(nanstd(overall_condSP)/sqrt(length(overall_condSP)))]);

save(['Mat/mturk_' expname '_results.mat'],'VHhumanStats','overall','overall_condG','overall_condSP');
