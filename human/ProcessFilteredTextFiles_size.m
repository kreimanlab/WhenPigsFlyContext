...
T = load('Mat/jasonsizecombined.mat');
T= T.jasonsizecombined;
...
%remove repeative items in _good.txt files
[content, ind] = unique(imagelist);

classlist = classlist(ind);
apartmentlist = apartmentlist(ind);
roomlist = roomlist(ind);
imagelist= imagelist(ind);
leftlist = leftlist(ind);
rightlist= rightlist(ind);
toplist = toplist(ind);
bottomlist = bottomlist(ind);
...
%     end
    ...
    for i= 1:length(whereapt)
        ...
        k = strfind(imageID,"_size_modified_");
        sz = char(imageID);
        sz = sz(k+length('_size_modified_'));
        infor.condSize = str2num(sz);
...
save('Mat/VHhumanStats_size.mat','VHhumanStats','countAll','UniqueRoomList','filtered_class');
