...
classnamelist =string( extractfield(VHhumanStats,'classname')');
...
headerimg = 'http://kreimanlab.com/mengmiMturkHost/VirtualHome/GT2/';
...
for i = 1:length(mturkData)
    ...
    if length(ans)<25 || length(dummyans) <3
    ...
    if length(selectedind) < 14 %below half of the trials
        mturkData(i).subjvalid = 0;
        continue;
    end
...
save(['Mat/mturk_GTlabel_compiled.mat'], 'mturkData');
...
save(['Mat/VHhumanStats_GTresult.mat'], 'VHhumanStats');
