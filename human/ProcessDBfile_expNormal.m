...
csvname = 'csv/results_normal_followup.csv';
system(['sqlite3 -header -csv db/expNormal_followup.db "select * from expNormal_followup;" > ' csvname]);

fid = fopen(csvname);
out = textscan(fid,'%s');
strtotal = out{1,1};
pattern = 'http://kreimanlab.com/mengmiMturkHost/VirtualHome/keyframe_VH_GT_gif/';

savefilename = 'Mat/mturk_expNormal_followup.mat';
...
%% combine previous pilot study
%% ONLY RUN ONCE
% load(savefilename);
% FmturkData = mturkData;
% 
% load('Mat/mturk_expNormal.mat');
% mturkData = [mturkData FmturkData];
% 
% save('Mat/mturk_expNormal.mat','mturkData');
